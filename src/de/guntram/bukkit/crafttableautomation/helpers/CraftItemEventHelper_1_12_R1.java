 ...
 * @author gbl
 */
public class CraftItemEventHelper_1_12_R1 implements CraftItemEventHelper {
    ...
    public Collection<ItemStack> getIngredientCollection(Recipe recipe) {
        getLogger().log(Level.FINE, "recipe is {0}", recipe.toString());
        if (recipe instanceof CraftShapedRecipe) {
            Map<Character, ItemStack> map=((CraftShapedRecipe)recipe).getIngredientMap();
            getLogger().log(Level.FINE, "map is {0}", map.toString());
            return map.values();
        } else if (recipe instanceof CraftShapelessRecipe) {
            return ((CraftShapelessRecipe)recipe).getIngredientList();
        } else {
            getLogger().log(Level.WARNING, "recipe class is {0}", recipe.getClass().getCanonicalName());
            return null;
        }
    }
}
